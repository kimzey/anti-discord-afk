...
ไม่ชอบไอคอนขยับไปมา ติ๊ก **Animate icon** ออกได้ — ดู [ผลกระทบต่อเครื่อง](#-กินทรัพยากรเครื่องแค่ไหน) ประกอบ

---

## ⚡ กินทรัพยากรเครื่องแค่ไหน

วัดจาก `.app` ตัวจริง โดยจับ CPU time ที่เพิ่มขึ้นในหน้าต่าง 60 วินาที หลังแอปนิ่งแล้ว 30 วินาที (ไม่เอาช่วง startup มาปน):

| โหมด | CPU | RAM |
|---|---|---|
| **default** (ไอคอนหมุน 2 เฟรม/วิ) | **1.8%** | 109 MB |
| ปิด `Animate icon` | **0.0%** | 105 MB |
| `Pause` | **0.0%** | 103 MB |

CPU ทั้งหมดมาจากการวาดไอคอนบน menu bar ไม่ใช่ตัวเช็ค idle — ตัวเช็คตื่นแค่ 5 วินาทีครั้ง อ่านตัวเลขจากระบบแล้วนอนต่อ วัดไม่ติดเลย

ถ้าอยากประหยัดกว่านี้ แก้ `FRAME_INTERVAL` ใน `menubar.py` — CPU แปรผันตรงกับความถี่:

| FRAME_INTERVAL | | CPU |
|---|---|---|
| `0.25` | 4 เฟรม/วิ | 3.1% |
| `0.5` | 2 เฟรม/วิ (default) | 1.6% |
| `1.0` | 1 เฟรม/วิ | 0.7% |

> เทียบให้เห็นภาพ: ตอนวัด Discord กิน 11%, Brave 18% — แอปนี้ 1.8% ถือว่าจิ๊บจ๊อย
> แต่ถ้าใช้แบตอยู่แล้วอยากรีดทุกเปอร์เซ็นต์ ปิด `Animate icon` ได้ 0% เลย

**RAM 109 MB** ลดไม่ได้ — py2app ห่อ Python runtime + PyObjC ไว้ทั้งก้อนเพื่อให้เครื่องปลายทางไม่ต้องมี Python
